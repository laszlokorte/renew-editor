...
export class LiveState {
	...
	join() {
		if (!this.joined) {
			...
			this.presence.onSync(() => {
				const newList = this.presence.list((id, { metas }) => {
				...
				this.eventTarget.dispatchEvent(
					new CustomEvent('presence-changed', {
						detail: newList
					})
				);
			});
	...
	emitError(type, error) {
		this.eventTarget.dispatchEvent(
			new CustomEvent('livestate-error', {
				detail: {
					type,
	...
	handleChange({ state, version }) {
		...
		this.eventTarget.dispatchEvent(
			new CustomEvent('livestate-change', {
				detail: {
					state: this.state,
	...
	handlePatch({ patch, version }) {
		this.eventTarget.dispatchEvent(
			new CustomEvent('livestate-patch', {
