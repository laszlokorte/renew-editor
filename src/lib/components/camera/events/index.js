...
import { atom, view, read, combine, combineWithRest } from '$lib/reactivity/atom.svelte.js';
...
export function bindEvents(node, { zoomDelta, rotationDelta, panScreenDelta, eventToWorld }) {
	...
	const disposePointer = bindPointerEvents(node, {
		eventToWorld,
		mouseGrab,
		rotationDelta,
		zoomDelta,
	...
	const disposeGesture = nativeGestureEvents
		? bindGestureEvents(node, { eventToWorld, rotationDelta, zoomDelta, panScreenDelta })
		: nativeTouchEvents
			? bindTouchEvents(node, { eventToWorld, rotationDelta, zoomDelta, panScreenDelta })
			: () => {};
