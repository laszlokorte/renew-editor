...
export function bindTouchEvents(
	element,
	{ eventToWorld, rotationDelta, zoomDelta, panScreenDelta }
) {
	let localTouches = [];
	let touchBaseRot = 0;
	...
	function onTouchMoveGlobal(evt) {
		if (evt.touches.length > 1 && localTouches.length > 0) {
			...
			const dw =
				(Math.atan2(Math.sin(newAngle - touchBaseRot), Math.cos(newAngle - touchBaseRot)) * 180) /
				Math.PI;
	...
	element.addEventListener('touchstart', onTouchStartLocal, { capture: true, passive: true });
	element.addEventListener('touchmove', onTouchMoveLocal, { capture: true, passive: true });
	...
	return () => {
		...
		element.removeEventListener('touchmove', onTouchMoveLocal, { capture: true, passive: true });
		element.removeEventListener('touchstart', onTouchStartLocal, { capture: true, passive: true });
	};
}
