...
import {
	...
	zoomWithPivot,
	zoomWithPivotScreen,
	zoomWithPivotZeroDelta,
	rotateWithPivotZeroDelta,
	panWithPivotZeroDelta,
	rotateWithPivotScreen,
	panWithPivotScreen
} from './navigation';
...
export const frameBoxLens = L.reread((camera) => {
	...
	return {
		...
		worldSpace: {
			a: Geo.rotatePivotXYDegree(camera.focus.x, camera.focus.y, -camera.focus.w, {
				x: minX,
				y: minY
			}),
			b: Geo.rotatePivotXYDegree(camera.focus.x, camera.focus.y, -camera.focus.w, {
				x: minX + width,
				y: minY
			}),
			c: Geo.rotatePivotXYDegree(camera.focus.x, camera.focus.y, -camera.focus.w, {
				x: minX + width,
				y: minY + height
			}),
			d: Geo.rotatePivotXYDegree(camera.focus.x, camera.focus.y, -camera.focus.w, {
				x: minX,
				y: minY + height
			})
		}
...
export const rotateMovementLens = [
	'focus',
	L.props('w', 'x', 'y'),
	L.lens(R.compose(R.assoc('dw', 0), R.zipObj(['px', 'py']), R.props(['x', 'y'])), rotateWithPivot)
];

export const zoomMovementLens = [
	'focus',
	L.props('z', 'x', 'y'),
	L.lens(R.compose(R.assoc('dz', 0), R.zipObj(['px', 'py']), R.props(['x', 'y'])), zoomWithPivot)
];
