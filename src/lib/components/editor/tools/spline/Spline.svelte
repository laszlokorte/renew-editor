<script>
	...
	import { fsm } from '$lib/reactivity/fsm.svelte.js';
	import { onPointerClick } from '$lib/reactivity/bindings.svelte.js';
	import { createMachine, assign, emit } from 'xstate';

	const { frameBoxPath, clientToCanvas, cameraScale, rotationTransform, onDraw } = $props();
				...
				on: {
					MOVE: {
						...
						actions: assign({
							...
							draftBackHandle: ({ event: { position }, context }) =>
								context.detached
					...
					TOGGLE_ATTACHMENT: {
						...
						actions: assign({
							draftBackHandle: ({
								context: { draftPoint, draftFrontHandle, detached, draftBackHandle }
							}) =>
								detached
									? {
											x: draftPoint.x * 2 - draftFrontHandle.x,
	...
	const currentDraft = read(
		L.pick({
			...
			pathNeck: [
				'path',
				L.reread((x) => (x.length > 0 ? x[Math.max(0, x.length - 2)] : undefined))
			],
...
<g
	...
	onpointermove={(evt) => {
		...
		if (evt.pointerId && !evt.currentTarget.hasPointerCapture(evt.pointerId)) {
			return;
		}
		...
		{#key 'pop'}
			{#if currentDraftValue.pathNeck && canAction.pop}
				...
				>
					...
					{:else}
						<rect
							x={currentDraftValue.pathNeck.point.x - 10 * cameraScaleValue}
							y={currentDraftValue.pathNeck.point.y - 10 * cameraScaleValue}
		...
		{#key 'finishOpen'}
			{#if canAction.finishOpen}
				...
				>
					...
					{:else}
						<rect
							x={currentDraftValue.pathHead.point.x - 10 * cameraScaleValue}
							y={currentDraftValue.pathHead.point.y - 10 * cameraScaleValue}
							width={20 * cameraScaleValue}
							height={20 * cameraScaleValue}
							class="finish"
						/>
		...
		{#key 'finishClose'}
			{#if canAction.finishClose}
				...
				>
					...
					{:else}
						<rect
							x={currentDraftValue.pathRoot.point.x - 10 * cameraScaleValue}
							y={currentDraftValue.pathRoot.point.y - 10 * cameraScaleValue}
							width={20 * cameraScaleValue}
							height={20 * cameraScaleValue}
							class="close"
						/>
			...
			{#if currentDraftValue.draftPoint && currentDraftValue.draftBackHandle}
				...
				>
					<circle
						...
						cy={currentDraftValue.draftBackHandle.y}
						r={10 * cameraScaleValue}
						class={{ detach: true, active: currentDraftValue.isDetached }}
						pointer-events="all"
					/>
					<circle
						cx={currentDraftValue.draftBackHandle.x}
