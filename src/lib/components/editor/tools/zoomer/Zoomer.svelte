<script>
	...
	const { frameBoxPath, clientToCanvas, onZoom, rotationTransform, cameraScale } = $props();
	...
	const zoomRefClient = view([L.removable('refClient'), 'refClient', L.removable('x', 'y')], zoom);

	const zoomRefWorld = view(
		['refClient', L.reread((refClient) => clientToCanvas(refClient.x, refClient.y))],
		zoom
	);

	const zoomPivotCurrentWorld = read(
		['pivotClient', L.reread((pivotClient) => clientToCanvas(pivotClient.x, pivotClient.y))],
		zoom
	);
	const isActive = view(
	...
	const piePath = view(
		({ pivotWorld, currentWorld, scale }) => {
			if (Math.hypot(pivotWorld.y - currentWorld.y, pivotWorld.x - currentWorld.x) / scale < 55) {
				return undefined;
			}
			...
			return `
				...
				L${
					...
					scale *
						...
						Math.cos(
							-Math.PI / 2 +
								Math.atan2(pivotWorld.y - currentWorld.y, pivotWorld.x - currentWorld.x)
						)
				} ${
					currentWorld.y -
					scale *
...
<path
	...
	onpointermove={(evt) => {
		...
		if (distance > minRadius) {
			const angle = Geo.angleRadBetween(zoomRefClient.value, zoomPivotClient.value, newPos);
...
<g transform={rotationTransform.value} pointer-events="none">
	{#if isActive.value}
		...
		<circle cx={refWorld.x} cy={refWorld.y} r={scale * 5} class="ref" fill="#4477aa" />
		<circle
			cx={currentWorld.x}
			cy={currentWorld.y}
			r={scale * 40}
		...
		<circle cx={currentWorld.x} cy={currentWorld.y} r={scale * 3} class="ref" fill="#4477aa" />
		<circle cx={currentWorld.x} cy={currentWorld.y} r={scale * 3} class="ref" fill="#4477aa" />
		<line
			x1={currentWorld.x}
			x2={refWorld.x}
