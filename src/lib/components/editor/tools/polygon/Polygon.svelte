<script>
	...
	const closeDraft = view(
		[
			L.pick({
				pos: 'pos',
				type: ['type', L.define('close'), L.normalize(R.always('close'))]
			}),
			'pos'
		],
		draft
	...
	const finishDraft = view(
		[
			L.pick({
				pos: 'pos',
				type: ['type', L.define('finish'), L.normalize(R.always('finish'))]
			}),
			'pos'
		],
		draft
	...
	const draftSnappedFinish = view(
		['type', L.lens(R.equals('finish'), (force, old) => (force ? 'finish' : old))],
		draft
	);
	const draftSnappedClose = view(
		['type', L.lens(R.equals('close'), (force, old) => (force ? 'close' : old))],
		draft
	);
	const draftSnappedPop = view(
		['type', L.lens(R.equals('pop'), (force, old) => (force ? 'pop' : old))],
		draft
	);

	const startPath = view([L.index(0), L.defaults(false)], path);
	const currentPath = view([L.appendTo, L.removable('x', 'y'), L.defaults(false)], path);

	const pathPath = view(
		L.iso(
			R.ifElse(
				R.length,
				R.compose(R.join(' '), R.map(R.compose(R.join(','), R.props(['x', 'y'])))),
				R.always('')
			),
			R.compose(R.map(R.compose(R.zipWith(R.assoc, ['x', 'y']), R.split(','))), R.split(' '))
		),
		path
	);
	...
	const draftPath = read(
		({ p, d }) =>
			p && d
				? R.join(', ', R.props(['x', 'y'], p)) + ' ' + R.join(', ', R.props(['x', 'y'], d))
				: '0,0 0,0',
...
<path
	...
	onpointerdown={(evt) => {
		...
		if (!startPath.value) {
			...
			Math.hypot(pathRoot.value.x - p.x, pathRoot.value.y - p.y) <
				cameraScale.value * (snapRadius + Math.hypot(evt.width, evt.height) / 2)
		) {
			path.value = [];
			return;
		}
	...
	onpointerup={(evt) => {
		...
		if (draftSnappedFinish.value) {
			...
			pathRoot.value &&
			Math.hypot(pathRoot.value.x - draftPos.value.x, pathRoot.value.y - draftPos.value.y) >
				cameraScale.value * snapRadius + Math.hypot(evt.width, evt.height) / 2
		) {
