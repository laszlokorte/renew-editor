...
export const isoTranslation = (d) =>
	L.iso(R.partial(translateXY, [d.x, d.y]), R.partial(translateXY, [-d.x, -d.y]));
export const isoScale = (s) =>
	L.iso(R.partial(scaleXY, [s.x, s.y]), R.partial(scaleXY, [1 / s.x, 1 / s.y]));
export const isoRotationDegree = (deg) =>
	L.iso(R.partial(rotateDegree, [deg]), R.partial(rotateDegree, [-deg]));
export const isoRotationRad = (rad) =>
	L.iso(R.partial(rotateRad, [rad]), R.partial(rotateRad, [-rad]));
export const isoScalePivot = (pivot, s) =>
	L.compose(L.inverse(isoTranslation(pivot)), isoScale(s), isoTranslation(pivot));
export const isoRotationPivotRad = (pivot, rad) =>
	L.compose(L.inverse(isoTranslation(pivot)), isoRotationRad(rad), isoTranslation(pivot));
export const isoRotationPivotDegree = (pivot, degree) =>
	L.compose(L.inverse(isoTranslation(pivot)), isoRotationRad(degree), isoTranslation(pivot));
...
function RayToLineSegment(x, y, dx, dy, { x: x1, y: y1 }, { x: x2, y: y2 }) {
